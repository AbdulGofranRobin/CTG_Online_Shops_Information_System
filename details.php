	...
	<div class="container">
		...
		<div class="row">
			<div class="col-md-8">
				...
				if($u=='m')
				{
					$rows=SelectAllWhere('market_info','market_id',$cat_id);
										foreach($rows as $row)
					{
						extract($row);
				...
				if($u=='ms')
				{
					$rows=SelectAllWhereTwo('shop_info','market_id','approve',$cat_id,1);
					$i=0;					
					foreach($rows as $row)
					{
						extract($row);
				...
				if($u=='s')
				{
					$rows=SelectAllWhereTwo('shop_info','shop_id','approve',$cat_id,1);
										foreach($rows as $row)
					$i=0;
					{
						extract($row);
